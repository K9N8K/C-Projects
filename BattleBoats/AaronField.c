...
FieldPosition FieldUpdateKnowledge(Field *f, const GuessData *gData)
{
    ...
    if(gData->hit == HIT_HIT)
    {
        ...
        {
            f->largeBoatLives = HIT_SUNK_HUGE_BOAT;
        }     
        else if(f->mediumBoatLives == 0)
        {
            f->mediumBoatLives = HIT_SUNK_HUGE_BOAT;
        }    
        else if(f->smallBoatLives == 0)
        {
            f->smallBoatLives = HIT_SUNK_HUGE_BOAT;
        }            
    }
    else
    {
...
{
    uint8_t states = 0;
    //uint8_t small = 0x01, medium = 0x02, large = 0x04, huge = 0x08;
